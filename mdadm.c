...
int mdadm_write(uint32_t addr, uint32_t len, const uint8_t *buf) {
  ...
  while(addr_copy < finish){
      ...
      // Seek to the correct disk and block, read, modify, and write back
      uint32_t op1 = use_addr(JBOD_SEEK_TO_DISK, dB, 0); 
      jbod_operation(op1, block);
      uint32_t op2 = use_addr(JBOD_SEEK_TO_BLOCK, 0, num_for_block);
      jbod_operation(op2, block);
      uint32_t op3 = use_addr(JBOD_READ_BLOCK, 0, 0);
      jbod_operation(op3, temporaryBuf); // Call jbod_operation to read the block into temporaryBuf
      
      uint8_t remaining_distance_to_process = finish - addr_copy; // Calculate remaining distance to process
